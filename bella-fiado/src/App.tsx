  ...
  return (<>
    {load && <Loader />}
    {
      ...
      <div className=' h-screen flex flex-col items-center justify-center bg-neutral-800 text-neutral-800'>
        ...
        <form className=' rounded p-4 bg-gradient-to-b from-orange-400 to-orange-500 w-4/5 md:w-2/3 lg:w-1/4 h-96 flex flex-col ' action="submit" onSubmit={sendLoginInfo}>
          <label className=' text-xl lg:text-2xl' htmlFor="username">Usuário:</label>
          <input autoComplete='off' onChange={e => setUsername(e.target.value)} placeholder='Digite seu usuário' maxLength={8} className=' border border-orange-600 mb-2 rounded text-xl lg:text-2xl px-2 pb-1' type="text" name='username' id='username' />
          <label className=' text-xl lg:text-2xl' htmlFor="password">Senha:</label>
          <input onChange={e => setPassword(e.target.value)} placeholder='****' className=' border border-orange-600 shadow-inner rounded text-xl lg:text-2xl px-2 pb-1' type="password" name='password' id='password' maxLength={4} />
