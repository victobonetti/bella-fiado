                        ...
                        {accountsData?.map((item) => {
                            return (
                                <div className=" rounded-xl p-8 mt-4 flex flex-col items-center justify-center bg-gradient-to-b from-gray-100 to-gray-300 text-neutral-700  ">
                                    <h1 className=" w-full text-start text-2xl">Conta {item._id}</h1>
                                    <h2 className=" w-full text-start text-xl">Foram pagos R${item.payments?.reduce((total, payment) => {
                                        return total + payment.value;
                                    }, 0).toFixed(2)}</h2>
                                    <h2 className="w-full text-start text-xl">O valor total da conta é R${item.items?.reduce((total, price) => {
                                    ...
                                    {itemsTableActive &&
                                        <table className=" w-full mt-4 table-auto border border-gray-400">
                                            ...
                                            <tbody>
                                                ...
                                                {item.items?.map((item) => {
                                                    return (
                                                        <tr className="text-center" key={item.product_id._id}>
                                                            <td className="border border-gray-400 px-4 py-2">{String(item.date).slice(0,10)}</td>
                                                            <td className="border border-gray-400 px-4 py-2">{item.product_id.name}</td>
                                                            <td className="border border-gray-400 px-4 py-2">R${item.product_id.price.toFixed(2)}</td>
                                                            <td className="border border-gray-400 px-4 py-2">{item.amount}</td>
                                                            <td className="border border-gray-400 px-4 py-2">R${(item.product_id.price * item.amount).toFixed(2)}</td>
                                                        </tr>
                                                    )
                                                })}
                                            </tbody>
